        ...
        <q-table :filter="filter" separator="horizontal" flat bordered :columns="columns" :data="getInstallments">
            ...
            <template v-slot:no-data>
                <div class="full-width row flex-center text-grey-9 text-h6 q-py-xl">
                   لا توجد أيه أقساط قرب موعد سدادها.
                </div>
            </template>
        </q-table>
    </div>
...
export default {
    ...
    computed: {
        columns: function(){
            return this.$store.state.informatics.installments.columns
        },
        getInstallments: function(){
            return this.$store.state.informatics.installments.data
        }
    }
}
</script>